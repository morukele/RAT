...
pub fn run(
    ...
    if command_with_args.is_empty() {
        return Err(error::Error::Internal("Command is not valid".to_string()));
    }
...
> {
    ...
    // encrypt job
    let cipher = XChaCha20Poly1305::new(key.as_ref().into());
    let encrypted_job = cipher
        .encrypt(&nonce.into(), encrypted_job_json.as_ref())
        .map_err(|e| error::Error::Internal(e.to_string()))?;
